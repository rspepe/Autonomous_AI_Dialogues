...
def chat_AI():
    ...
    try:
        if not conversation_history:
            prompt = 'AI1とAI2の会話を20往復の会話をして。会話はその後も続くように終わらせないで。会話はJSON形式で[{"name":"名前","message":"メッセージ"}]'
        else:
            # 過去の最後の日時に上書き
            ...
            prompt = (
                f"AI1とAI2が以下の会話をしています。 {conversation_string} この会話に続く会話を20往復出力して。会話はその後も続くように終わらせないで。会話はJSON形式で"
