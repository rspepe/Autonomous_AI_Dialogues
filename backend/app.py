...
def chat_AI():
    ...
    try:
        if not conversation_history:
            ...
            prompt = (
                f"AI1とAI2が以下の会話をしています。 {conversation_string} この会話に続く会話を20往復出力して。口調はフレンドリーで。30文字以内で。同じような会話が続かないように。「スマホ」「海外旅行」「ゲーム」「プログラミング」「料理」のどれかの話題をたまに入れて。会話はJSON形式で"
        ...
        for i, entry in enumerate(response):
            entry["visible_from"] = (
                start_datetime + datetime.timedelta(minutes=i * 3)
            ).isoformat()
